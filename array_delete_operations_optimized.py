def solve_array_delete():
    ...
    for _ in range(T):
        ...
        # 计算最少删除操作次数
        result = min_delete_operations_optimized(n, arr)
        print(result)
...
def min_delete_operations_brute_force(n, arr):
    """
    暴力搜索版本（用于对比）
    """
    if n == 0:
...
def test_examples():
    ...
    if is_monotonic_increasing(arr):
        ...
        # 使用优化算法
        result1 = min_delete_operations_optimized(n, arr)
    ...
    if is_monotonic_increasing(arr):
        ...
        result3 = min_delete_operations_optimized(n, arr)
    ...
    # 对比两种算法
    print("\n算法对比:")
    print("暴力搜索版本结果:", min_delete_operations_brute_force(n, arr))
